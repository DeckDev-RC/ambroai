...
## Ao Exibir, Traduza
"ml" -> "Mercado Livre" | "bagy" -> "Bagy" | "physical store" -> "Loja Fisica"
...
- ordersByMarketplace: vendas por canal

TEMPORAIS:
- salesByMonth: evolucao mensal (mes a mes)
...
- marketplaceGrowth: evolucao mensal de cada marketplace
...
7. Nunca exponha IDs ou dados tecnicos
...
13. Quando gerar tabelas, use formato markdown com | e alinhamento
  ...
  try {
    switch (fnName) {
      ...
      case "ordersByMarketplace": {
        const mkt = r.marketplaces as Record<string, { count: number; total: number }>;
        ...
        return "**Por marketplace** (" + fNum((r.total_orders as number) || 0) + "):\n\n" +
          entries.map(([n, d]) => "- **" + (mPT[n] || n) + ":** " + fBRL(d.total) + " | " + fNum(d.count) + " pedidos").join("\n") + chartBlock;
      }

      case "salesByMonth": {
        const months = r.months as Array<{ month: string; count: number; total: number; avg_ticket: number; growth_pct: number | null }>;
        ...
        return "**Evolucao mensal** (total: " + fBRL((r.grand_total as number) || 0) + "):\n\n" +
          months.map((m) => {
      ...
      case "executiveSummary": {
        ...
        const hl = r.health as Record<string, unknown>;
