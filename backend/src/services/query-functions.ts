...
export async function countOrders(params: QueryParams) {
  ...
  return { total: count || 0, filters: { status: params.status, marketplace: params.marketplace, period: fmtPeriod(dateRange) } };
}
...
export async function totalSales(params: QueryParams) {
  ...
  return { total_sales: Math.round(total * 100) / 100, order_count: data.length, filters: { status: params.status, marketplace: params.marketplace, period: fmtPeriod(dateRange) } };
}
...
export async function ordersByMarketplace(params: QueryParams) {
  const dateRange = buildDateRange(params);
  const data = await fetchAllRows<{ marketplace: string; total_amount: number }>("orders", "marketplace, total_amount", (q) => {
    let query = applyDateFilter(q, dateRange);
  ...
  data.forEach((row) => {
    ...
    grouped[m].count++;
    grouped[m].total += Number(row.total_amount) || 0;
  });
  for (const key of Object.keys(grouped)) {
    grouped[key].total = Math.round(grouped[key].total * 100) / 100;
  }
  return { marketplaces: grouped, total_orders: data.length, filters: { status: params.status, period: fmtPeriod(dateRange) } };
}
...
export async function salesByMonth(params: QueryParams) {
  ...
  data.forEach((row) => {
    ...
    if (!grouped[month]) grouped[month] = { count: 0, total: 0 };
    grouped[month].count++;
    grouped[month].total += Number(row.total_amount) || 0;
  });
